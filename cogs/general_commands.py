...
class GeneralCommands(commands.Cog):
    ...
    async def sobre_slash(self, interaction: discord.Interaction):
        ...
        embed_sobre.set_footer(text=f"Comando solicitado por: {interaction.user.name}")
    ...
    async def hora_slash(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f'Agora são {hora_formatada} do dia {data_formatada} aqui em Araçuaí.')
    ...
    # --- OUVINTE DE ERROS CORRIGIDO ---
    # Este método agora captura erros de todos os comandos de barra NESTE arquivo.
    async def cog_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, app_commands.MissingPermissions):
            await interaction.response.send_message("Você não tem permissão para usar este comando!", ephemeral=True)
        else:
            # Para outros erros, envia uma mensagem genérica e imprime o erro no console para depuração
            print(f"Ocorreu um erro não tratado no Cog 'GeneralCommands': {error}")
